...
double open_quad( double func(double), double leftEndPt, double rightEndPt, double absAcc, double relAcc, double* integrationError ){
    double transformInterval (double x){
        return 1.0/2*(rightEndPt - leftEndPt)*func( 1.0 / 2.0 * (leftEndPt + rightEndPt) + 1.0/2.0 * (rightEndPt - leftEndPt) * x );
    }
    double clenshawCurtisQuad  (double angle){
        return transformInterval(cos(angle))*sin(angle);
    }

    return adapt( clenshawCurtisQuad, 0, M_PI, absAcc, relAcc, integrationError );
}
